...
Check [squid website](http://www.squid-cache.org/Intro/)

## [Why you'd use it](http://www.squid-cache.org/Intro/why.html)

## How to use
> Note **If you're using windows, you need to run following commands without sudo**

build image or pull from dockerhub

**Squid configuration options**

you can find options that's supported with our image in `configs` file
to use any of these flags,in `configs_switch` assign 1 to the flag name. 
...
To use external file, copy it config/ directory in this repository and it will be copied to the container. then you need to [activate this file](#Activate-a-specific-config-file)
